...
static void uart_mtk_poll_out (const struct device*  dev,
                               unsigned char         c);
static int uart_mtk_poll_in (const struct device*  dev,
                             unsigned char*        c);

#ifdef CONFIG_UART_INTERRUPT_DRIVEN
static int uart_mtk_fifo_fill (const struct device*  dev,
                               const uint8_t*        tx_data,
                               int                   size);
static int uart_mtk_fifo_read (const struct device*  dev,
                               uint8_t*              rx_data,
                               const int             size);
...
static void uart_mtk_irq_callback_set (const struct device*           dev,
                               ...
                               int                   size)
{
    ...
    while ((idx < size)                           &&
           (is_tx_avail (DEVICE_MMIO_GET (dev))))
    {
    	/* Send the character */
				               ...
				               const int             size)
{
    ...
    while ((idx < size)                           &&
           (is_rx_avail (DEVICE_MMIO_GET (dev))))
    {
        rx_data [idx] = (uint8_t) (sys_read32 (DEVICE_MMIO_GET (dev) + UART_OFFSET_RBR) & 0x000000ff);
...
static void uart_mtk_irq_tx_enable (const struct device*  dev)
{
    ...
    {
    	sys_write32 ((uint32_t) (ier | UART_IER_TX_INTERRUPT), (DEVICE_MMIO_GET (dev) + UART_OFFSET_IER));
    }
...
static void uart_mtk_irq_rx_enable (const struct device*  dev)
{
    ...
    {
    	sys_write32 ((uint32_t) (ier | UART_IER_RX_INTERRUPT), (DEVICE_MMIO_GET (dev) + UART_OFFSET_IER));
    }
...
static int uart_mtk_irq_is_pending (const struct device*  dev)
{
    ...
    return ((((iir & UART_IIR_RX_INTERRUPT) != 0) || ((iir & UART_IIR_TX_INTERRUPT) != 0)) ? 1 : 0);
}
...
static int uart_mtk_init (const struct device* dev)
{
    ...
    sys_write32 ((uint32_t) (UART_FCR_RX_FIFO_1 | UART_FCR_TX_FIFO_4 | UART_FCR_CLR_TX_FIFO | UART_FCR_CLR_RX_FIFO | UART_FCR_FIFO_EN), (DEVICE_MMIO_GET (dev) + UART_OFFSET_FCR));
...
#define UART_IRQ_CONFIG_FUNC(n)                                                                             \
	...
	{                                                                                                       \
		IRQ_CONNECT (DT_INST_IRQN (n), DT_INST_IRQ (n, priority), uart_mtk_isr, DEVICE_DT_INST_GET (n), 0); \
